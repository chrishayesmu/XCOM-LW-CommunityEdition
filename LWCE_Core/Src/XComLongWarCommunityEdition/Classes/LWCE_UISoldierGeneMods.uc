class LWCE_UISoldierGeneMods extends UISoldierGeneMods;
...
simulated function Init(XGStrategySoldier kSoldier, XComPlayerController _controllerRef, UIFxsMovie _manager, bool viewGenesOnly, int iSelectedRow, int iSelectedCol)
{
    ...
    if (!`HQPRES.IsMgrRegistered(class'LWCE_XGSoldierUI'))
    {
        m_kLocalMgr = Spawn(class'LWCE_XGSoldierUI', XComHQPresentationLayer(controllerRef.m_Pres));
...
event Destroyed()
{
    ...
    if (!m_bViewOnly)
    {
        `HQPRES.RemoveMgr(class'LWCE_XGSoldierUI');
    }
