...
simulated function BuildAbilities(XGUnit kUnit)
{
    ...
    `LWCE_LOG_CLS("BuildAbilities");

    forceAbilitySlot = 0;

    if (UITacticalHUD(screen).IsMenuRaised())
    {
        `LWCE_LOG_CLS("Menu raised");
    ...
    `LWCE_LOG_CLS("Unit has " $ Len $ " abilities");
    ...
    `LWCE_LOG_CLS("After loop: m_arrAbilities.Length = " $ m_arrAbilities.Length);
