...
function RealizeSelected()
{
    ...
    local string tmpStr;
    local Highlander_XGHangarUI kMgr;
    local TShipWeapon kWeapon;

    kMgr = Highlander_XGHangarUI(GetMgr());

    kWeapon = kMgr.SHIPWEAPON(`HL_HANGAR.HL_ItemTypeToShipWeapon(kMgr.m_arrHLItems[m_iCurrentSelection].iItemId));
