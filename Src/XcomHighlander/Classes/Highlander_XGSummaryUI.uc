class Highlander_XGSummaryUI extends XGSummaryUI;

function TTableMenuOption BuildArtifactLine(EItemType eItem, int iQuantity)
{
    local TTableMenuOption kOption;

    `HL_LOG_DEPRECATED_CLS(BuildArtifactLine);

    return kOption;
}

function TTableMenuOption HL_BuildArtifactLine(int iItemId, int iQuantity)
{
...
function CollectArtifactsFromDeadAliens()
{
    ...
    local HighlanderItemContainer kCargoItems, kDescItems;

    kDesc = Highlander_XGBattleDesc(Desc());
    kDescItems = kDesc.m_kArtifactsContainer;

    kCargo = Highlander_XGDropshipCargoInfo(kDesc.m_kDropShipCargoInfo);
...
{
    local HighlanderItemContainer kArtifactsContainer;
    local HL_TItemQuantity kItemQuantity;
    local TTableMenu kTable;

    `HL_LOG_CLS("UpdateArtifacts");

    kArtifactsContainer = Highlander_XGDropshipCargoInfo(Desc().m_kDropShipCargoInfo).m_kArtifactsContainer;
    ...
    foreach kArtifactsContainer.m_arrEntries(kItemQuantity)
    {
        ...
        {
            kTable.arrOptions.AddItem(HL_BuildArtifactLine(kItemQuantity.iItemId, kItemQuantity.iQuantity));
        }
    }
