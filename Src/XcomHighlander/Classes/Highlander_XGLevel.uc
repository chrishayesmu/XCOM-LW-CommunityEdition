class Highlander_XGLevel extends XGLevel;
...
simulated function OnStreamingFinished()
{
    ...
    `HL_LOG_CLS("OnStreamingFinished");
    ...
    {
        `HL_LOG_CLS("Before InitCollectibles: BattleSP.m_kDesc.m_arrArtifacts.Length = " $ BattleSP.m_kDesc.m_arrArtifacts.Length);
        class'XComCollectible'.static.InitCollectibles(BattleSP.m_kDesc.m_arrArtifacts);
        `HL_LOG_CLS("After InitCollectibles: BattleSP.m_kDesc.m_arrArtifacts.Length = " $ BattleSP.m_kDesc.m_arrArtifacts.Length);

        for (Index = 0; Index < BattleSP.m_kDesc.m_arrArtifacts.Length; Index++)
        {
            if (BattleSP.m_kDesc.m_arrArtifacts[Index] > 0)
            {
                `HL_LOG_CLS("Artifact " $ Index $ " has quantity " $ BattleSP.m_kDesc.m_arrArtifacts[Index]);
            }
        }
    }
