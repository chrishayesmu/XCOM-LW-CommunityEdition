class Highlander_UISoldierLoadout extends UISoldierLoadout;
...
simulated function Init(XGStrategySoldier kSoldier, XComPlayerController _controllerRef, UIFxsMovie _manager)
{
    ...
    if (!XComHQPresentationLayer(controllerRef.m_Pres).IsMgrRegistered(class'Highlander_XGSoldierUI'))
    {
...
simulated function XGSoldierUI GetMgr(optional int iStaringView = -1)
{
    if (m_kLocalMgr == none)
    {
        m_kLocalMgr = XGSoldierUI(XComHQPresentationLayer(controllerRef.m_Pres).GetMgr(class'Highlander_XGSoldierUI', self));
    }
...
simulated function ShowItemCard()
{
    ...
    cardData = Highlander_XGSoldierUI(GetMgr()).HL_SOLDIERUIGetItemCard();

    if (cardData.iCardType != 0)
    {
        PlaySound(`SoundCue("SoundUI.MenuSelectCue"), true);
        `HL_HQPRES.HL_UIItemCard(cardData);
...
simulated function UpdateInventoryList()
{
    ...
    local Highlander_XGSoldierUI kMgr;
    local TInventoryOption kOption;

    Invoke("ClearInventoryList");
    kMgr = Highlander_XGSoldierUI(GetMgr());
    ...
    for (I = 0; I < Length; I++)
    {
        ...
        AS_AddInventoryItem(Type, kOption.txtName.StrValue, imgPath, kMgr.HL_GetItemCharges(kOption.iItem, true), GetMecArmorIconsArray(kOption.iItem));
    }
}
